...
{
    cv::Mat(3,3, CV_32F, const_cast<float*>(&calibration.getIntrinsic()[0][0])).copyTo(camMatrix);
    cv::Mat(4,1, CV_32F, const_cast<float*>(&calibration.getDistorsion()[0])).copyTo(distCoeff);
...
bool MarkerDetector::findMarkers(const BGRAVideoFrame& frame, std::vector<Marker>& detectedMarkers)
{
    ...
    convert2Gray(bgraMat, m_grayscaleImage);
...
void MarkerDetector::convert2Gray(const cv::Mat& bgraMat, cv::Mat& grayscale) const
{
...
void MarkerDetector::estimatePosition(std::vector<Marker>& detectedMarkers)
{
    for (size_t i=0; i<detectedMarkers.size(); i++)
    {
        ...
        for (int col=0; col<3; col++)
        {
            ...
            {
                m.transformation.r()[row][col] = rotMat(row,col); // rotation
            }
            m.transformation.t()[col] = Tvec(col); // translation
        }
